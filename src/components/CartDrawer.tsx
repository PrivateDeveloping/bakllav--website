...
import { eur, priceFor, productName, sizeLabel, variantLabel } from "@/config/business";
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Varianti: {variantLabel(it.variantId)} · Madhësia: {sizeLabel(it.size)} ·{" "}
                      {eur(priceFor(it.size, it.variantId))} / tepsi
                    </p>
                  </div>
                  <p className="font-serif text-lg tabular-nums">
                    {eur(priceFor(it.size, it.variantId) * it.qty)}
                  </p>
                </div>
