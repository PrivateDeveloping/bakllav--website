...
//#region node_modules/.nitro/vite/services/ssr/assets/orders.server-BcrNhUZV.js
  ...
  <body style="margin:0; background:#f7f2e8; color:#2c1d14; font-family:Arial, sans-serif;">
    <div style="max-width:720px; margin:0 auto; padding:28px 18px;">
      <div style="background:#ffffff; border:1px solid #e5d8c5; padding:24px;">
        ...
        ${c.notes ? `<h2 style="margin:24px 0 10px; font-size:16px;">Shënime</h2>
...
async function deliverOrder(o) {
	...
	try {
		...
		if (error instanceof Error && error.message.startsWith("Missing required environment variable:")) throw error;
		console.error("[porosi] Resend order notification failed:", error);
		throw new Error("Order notification email could not be sent.");
	}
}
async function deliverMessage(m) {
	const resend = new Resend(requiredEnv("RESEND_API_KEY"));
	try {
		const { data, error } = await resend.emails.send({
			from: requiredEnv("ORDER_FROM_EMAIL"),
...
//#endregion
export { deliverMessage, deliverOrder };
